...
cipher_file = sys.argv[1] #python3 subsitutioncipher.py cipher.txt

"Dat file should not be changed as this will lead to a corrupt file, dat file has been regenerated fixing the pickle error (More details at bottom of program)"
...
"""NOTE : To test this program in terminal please use python3 subsitutioncipher.py (txt file) , I will leave cipher.txt empty if you wish to just paste the cipher text in there."""
...
if __name__ == '__main__':
    with open(cipher_file,encoding="utf8") as f:
        cypher_text = f.read()
    ...
    print("Please check inputfilename-decrypted.txt for decrypted message")
    print("Please check inputfilename-key.txt for decrypted key")
...
"Just for assurance, cipher.txt will be left in the same folder as this program and will have no text inside it.I will have all my test files in the tests repo which you can view to see how I tested this program"
"Please check inputfilename-decrypted.txt and inputfilename-key.txt for the output of this program"


"Updates on errors , if for some reason there is a format error its most likely dependent on the python version.Easy fix just comment out line of code where the format is and uncomment the line below it or just try another python version"

"Youll notice in my commits I have fixed pickle error, if you receive the error _pickle.UnpicklingError: invalid load key, '8'. , please do the following"
"(step 1)Make sure your in Version-for-grading directory in terminal"
"(step 2)Please Open python interpreter , type from substitutioncipher import build_english_frequency_dictionary and press enter"
"""(step 3)Now enter build_english_frequency_dictionary("Dictionary.txt") and press enter"""
"Try run the subsitutioncipher program again and if there is still the same error , please delete dictionary.dat file and do steps 1 to 3 again"
"This will regenerate the dat file , Im very sure you shouldnt get this error because I regenerated the dat file before submitting.Also any changes done to the dat file will corrupt it and will not be able execute the program"
